...
def connect_to_ad():
    app_log.message("debug", "Connecting to AD...")
    ...
    app_log.message("debug", "Connected.")
    return conn
...
def edit_ad_user(user_identifier, action, db_row):
    ...
    app_log.message("debug", f"Looking for user {user_identifier}")
    ...
    if not conn.entries:
        app_log.message("error", f"User {user_identifier} not found")
    ...
    if action == "away":
        app_log.message("debug", f"Adding {user_identifier} to Allow Access When Overseas")
        ...
        app_log.message("debug", f"Removing {user_identifier} from Block Access If Not In Australia")
        ...
        app_log.message("debug", f"Adding {user_identifier} to Enable Office 365 MFA")
        # add user to 2fa group
        conn.extend.microsoft.add_members_to_groups(
            user_dn, f'CN=Enable Office 365 MFA,OU=Office 365 and Azure AD,{DOMAIN_BASE}')
    elif action == "home":
        app_log.message("debug", f"Adding {user_identifier} to Block Access If Not In Australia")
        # add user to group that disables overseas access
        conn.extend.microsoft.add_members_to_groups(
            user_dn, f'CN=Azure - Block Access to O365 if not in Australia,OU=Office 365 and Azure AD,{DOMAIN_BASE}')
        
        app_log.message("debug", f"Removing {user_identifier} from Allow Access When Overseas")
        # remove user from overseas access group
        conn.extend.microsoft.remove_members_from_groups(
            user_dn, f'CN=Azure - Allow access when overseas (Travelling Users),OU=Office 365 and Azure AD,{DOMAIN_BASE}')
        
        app_log.message("debug", f"Checking if {user_identifier} is a staff member based on department: {user_department}")
        if user_department != "Staff":
            # remove students from 2fa group
            app_log.message("debug", f"{user_identifier} is not staff so removing from Enable Office 365 MFA")
            conn.extend.microsoft.remove_members_from_groups(
                user_dn, f'CN=Enable Office 365 MFA,OU=Office 365 and Azure AD,{DOMAIN_BASE}')

    conn.unbind()
    app_log.message("debug", f"Deleting backup record in schedules.sqlite")
